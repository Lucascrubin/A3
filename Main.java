...
class Main {
	...
	static void provaAI(double[] notas) {
		double aiNota = 0.0;
		int menorNota = notas[0] <= notas[1] ? 0 : 1; // Verifica a menor nota entre a A1 e A2.

		System.out.println("\nDigite a nota da Prova AI");
		aiNota = lerNota("\nAI", NOTA_MAX_AVALIACOES[menorNota]);
		System.out.println();

		if (aiNota > notas[menorNota]) {
			notas[menorNota] = aiNota;
	 ...
	 */
	static void aiOpcaoProva(double[] notas) {
